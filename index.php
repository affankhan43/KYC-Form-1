		...
		if(isset($_POST['kyc-submit'])){
			if(check_code($_POST['xss_code'])){
				if(!empty($_POST['first_name']) && !empty($_POST['last_name']) && !empty($_POST['street-line-1']) && !empty($_POST['city']) && !empty($_POST['state']) && !empty($_POST['country']) && !empty($_POST['phone']) && !empty($_POST['zip']) && !empty($_POST['passport_no']) && !empty($_POST['passport_exp']) && !empty($_POST['passport_isu'])){
					...
					if($_FILES['passports']['error']==0 && $_FILES['passport_selfie']['error']==0 && $_FILES['statement_bill']['error']==0){
						...
						$error[0] = "ERROR IN UPLOADING".json_encode($_FILES['passports']['error'],$_FILES['passport_selfie']['error'],$_FILES['statement_bill']['error']);
					}
				}
				else{
							...
							<div class="col-md-6">
								...
								<div class="row">
								 ...
								 <div class="alert alert-danger">
								 	FILE SIZE MUST BE LESS THAN <strong>1.5 MB</strong>
								 </div>
								<div class="row">
									<div class="form-group col-md-12">
